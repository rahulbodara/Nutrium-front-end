            ...
            <div className='relative mt-[10px]'>
                <div className='w-[100px] h-[34px] bg-white border border-[#eeeeee] min-w-[59px] min-h-[34px] text-white relative overflow-hidden rounded-[3px] inline-block text-center'>
                    ...
                    <div className={` ${isSwitchOn ? "left-0" : "-left-full"} absolute w-[200%] inset-y-0 select-none transition-[left] duration-[0.35s]`}>
                        {
                            isSwitchOn ? 
                            <label htmlFor='switcher' className={`absolute top-0 bottom-0 pr-[24px] pl-[12px] right-[50%] left-0 bg-[#1AB394] text-white inline-block align-middle min-h-[20px] cursor-pointer text-white p-[6px_12px] text-[14px] leading-[1.42] whitespace-nowrap select-none max-w-full`}>Private</label>:
                            <label htmlFor='switcher' className={`absolute top-0 bottom-0 bg-[#EEEEEE] right-0 left-[50%]  pl-[24px] border-[#bababa] text-[#676A6C]  inline-block align-middle min-h-[20px] cursor-pointer  p-[6px_12px] text-[14px] leading-[1.42] whitespace-nowrap select-none max-w-full`}>Public</label>
                        }
