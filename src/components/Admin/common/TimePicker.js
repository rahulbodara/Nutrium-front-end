          ...
          <div className="select-field flex-grow border-[#EEEEEE] relative border-[1px]">
            <span className={`p-[10px] block ${isFocused ? "" : "hidden"}`}>
              ...
              >
                ...
                {Array.from({ length: props?.hour ? props?.hour : 24 }).map(
                  (_, index) => (
              ...
              >
                ...
                {Array.from({
                  length: props?.min
                    ? props?.min
                    : 60 / (props?.minuteStep || 1),
                }).map((_, index) => (
                  <option key={index} value={index * (props?.minuteStep || 1)}>
                    {index * (props?.minuteStep || 1)}
                  </option>
                ))}
              </select>
            </span>
            <span className={`p-[10px] block ${!isFocused ? "" : "hidden"}`}>
              {props?.selectedHour ? props?.selectedHour : "00"}:
              {props?.selectedMinute ? props?.selectedMinute : "00"}
            </span>
          </div>
          <button
            type="button"
            ...
            } clr-grn flex cursor-pointer items-center align-middle bg-[#FAFAFB] p-2.5 border-l-[#EEEEEE] border-[1px]`}
          >
