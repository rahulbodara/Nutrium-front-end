...
import { CreateSchedule } from "@/redux/action/Schedule";
...
const SetShedule = ({ isSheduleOpen, setIsSheduleOpen }) => {
  ...
  const toggleCheckbox = (days) => {
    setDays((prevDays) =>
      prevDays.map((day) =>
        day.id === days.id ? { ...day, isChecked: !day.isChecked } : day
      )
    );
    setFormData({...formData,day:days.label})
  };
  const isAtLeastOneDayChecked = days?.some((day) => day.isChecked);
  const handleChange = (e)=>{
    const {name,value} = e.target;
    setFormData({...formData,isEnabled:isAtLeastOneDayChecked,[name]:value})
  } 
                                                ...
                                                <div className={`px-[30px] py-[20px] bg-[#fafafb] border-t-[1px] border-[#DDDDDD] ${index === dayList.length - 1 ? 'border-b-[1px]' : ''}`} key={index}>
                                                    <div className='flex items-center'>
                                                        ...
                                                        <div className='text-end'>
                                                            <Switcher id={day.id} label1="Enabled" label2="Disabled" checked={day.isChecked} onChange={() => toggleCheckbox(day)} />
                                                        </div>
                                                    </div>
                                                        ...
                                                        <div className='flex'>
                                                            <div className='flex flex-grow'>
                                                                ...
                                                                <div className='flex-grow basis-0 mr-[10px] w-[100px]'>
                                                                    ...
                                                                    <div>
                                                                        <input type={"time"} className='text-[14px] h-[32px] p-[5px] rounded-[5px] border-solid !border-[#AAAAAA] text-center focus:ring-0 bg-white input-transition focus:!border-[#1ab394] w-full' name="startTime" onChange={handleChange}/>
                                                                    </div>
                                                                </div>
                                                                <div className='flex-grow basis-0 w-[100px]'>
                                                                    <div className='text-[12px] font-light ml-[3px] mb-[3px]'>End time</div>
                                                                    <div>
                                                                        <input type={"time"} className='text-[14px] h-[32px] p-[5px] rounded-[5px] border-solid !border-[#AAAAAA] text-center focus:ring-0 bg-white input-transition focus:!border-[#1ab394] w-full' name="endTime" onChange={handleChange}/>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div className='flex items-center' onClick={() => toggleCheckbox(day)}>
